...
import RoomImageManager from "../components/owner/RoomImageManager";
...
const OwnerRoomsPage = () => {
  ...
  const [imageManagingRoom, setImageManagingRoom] = useState<Room | null>(null);
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      ...
      try {
        ...
        const data = await getRoomsByHotel(Number(hotelId));

        setRooms(data);
      } catch (error: unknown) {
        if (axios.isAxiosError(error)) {
          const message =
            error.response?.data?.detail ?? "Failed to load rooms.";

          setError(message);

          toast.error(message);
        } else {
          const message = "Failed to load rooms.";
  ...
  const handleCreateRoom = async (data: RoomCreate) => {
    try {
      ...
      setRooms((previousRooms) => [newRoom, ...previousRooms]);
  ...
  const handleUpdateRoom = async (data: RoomUpdate) => {
    if (!editingRoom) {
      return;
    }
    ...
    try {
      ...
      const updatedRoom = await updateRoom(editingRoom.id, data);
  ...
  const handleRoomImagesUpdated = (updatedRoom: Room) => {
    setRooms((previousRooms) =>
      previousRooms.map((room) =>
        room.id === updatedRoom.id ? updatedRoom : room,
      ),
    ...
    setImageManagingRoom(updatedRoom);
  };
  /* ---------- Loading ---------- */
    ...
    return (
      <main className="owner-rooms-page">
        <div className="owner-rooms-container">
          <div className="owner-rooms-loading">Loading rooms...</div>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="owner-rooms-page">
      <div className="owner-rooms-container">
        ...
        <header className="owner-rooms-header">
          <div>
            ...
            <span className="owner-rooms-badge">Hotel Owner</span>

            <h1>Room Management</h1>

            <p>Manage rooms for Hotel #{hotelId}.</p>
          </div>
        ...
        <section className="owner-rooms-section">
          <div className="owner-rooms-section-header">
            <div>
              ...
              <p>Rooms currently managed for this hotel.</p>
            </div>
          </div>
          ...
          {rooms.length === 0 ? (
            <div className="owner-rooms-empty">
              ...
              <p>This hotel does not have any rooms yet.</p>
            ...
            <div className="owner-rooms-grid">
              {rooms.map((room) => (
                <article key={room.id} className="owner-room-card">
                  {/* Room Header */}
                  ...
                  <div className="owner-room-card-actions">
                    ...
                    <button
                      type="button"
                      className="owner-room-image-button"
                      onClick={() => setImageManagingRoom(room)}
                      disabled={deletingRoomId === room.id}
                    >
                      <ImagePlus size={16} />
                      Images
      ...
      {imageManagingRoom && (
        <div className="owner-room-form-overlay">
          <RoomImageManager
            room={imageManagingRoom}
            onClose={() => setImageManagingRoom(null)}
            onRoomUpdated={handleRoomImagesUpdated}
          />
        </div>
      )}
    </main>
